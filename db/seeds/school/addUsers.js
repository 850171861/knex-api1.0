...
module.exports = async function (knex, ctx) {
  ...
  // 聖若瑟教區中學第一校（老师）
  users.push(
    ...generateCDSJ1TeachersUsers(uuid).map(doc => _.omit(doc, ['cls', 'courses', 'face_id', 'image_id']))
  )

  // 聖若瑟教區中學第六校（老师）
  users.push(
    ...generateCDSJ6TeachersUsers(uuid).map(doc => _.omit(doc, ['cls', 'courses', 'face_id', 'image_id']))
  )
