...
const { generateRegionAssessmentTemplates } = require('../data/region/assessmentTemplate')
let i = 0
...
const uuid = generateUUID('80000000')
...
module.exports = async function (knex, ctx) {
  ...
  const data = [
    ...assessmentTemplates,
    ...generateRegionAssessmentTemplates(uuid, ctx)
  ]
  // 資料庫操作
  await knex('assessment_templates').insert([
    ...data.map(doc => ({
      ...doc,
  ...
  // 讀取資料
  const docs = await knex.select().table('assessment_templates')

  // 返回
  ctx.assessmentTemplates = {
    docs,
    obj: _.keyBy(docs, (o) => {
      return o.name
    })
